...
const bookAppointment = async (req, res) => {
  try {
    ...
    // doctorId is Doctor._id (profile id)
    const doctorProfile = await Doctor.findById(doctorId).populate('user');
    if (!doctorProfile) {
      return res.status(404).json({ success: false, message: 'Doctor not found.' });
    }

    const idx = parseInt(clinicIndex) || 0;
    const clinic = doctorProfile.clinics[idx] || {};
    ...
    const appointment = await Appointment.create({
      ...
      timeSlot,
      symptoms: symptoms || '',
      fee: clinic.fee || doctorProfile.consultationFee || 0,
...
// @desc    Get pending appointments for assistant (payment_uploaded)
...
const completeAppointment = async (req, res) => {
  try {
    ...
    if (!appointment) {
      return res.status(404).json({ success: false, message: 'Appointment not found or not eligible to complete.' });
    }
