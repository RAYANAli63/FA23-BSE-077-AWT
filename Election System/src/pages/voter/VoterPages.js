...
export function VoterDashboard() {
  ...
  const statusMap = { registered: { label: '✅ Registered', badge: 'approved' }, finalized: { label: '🎫 Authorized', badge: 'active' }, waitlisted: { label: '📋 Waitlisted', badge: 'pending' }, voted: { label: '🗳️ Vote Cast', badge: 'completed' } };

  return (
    <div className="container page">
      <div style={{ marginBottom: '32px' }}>
        <h1>My Dashboard</h1>
        <p style={{ color: 'var(--text2)', marginTop: '4px' }}>Welcome, {profile?.full_name} 👋</p>
      </div>

      <div className="grid-4" style={{ marginBottom: '32px' }}>
        {[
          { label: 'Elections Joined', value: registrations.length, color: 'var(--accent)' },
          { label: 'Votes Cast', value: registrations.filter(r => r.status === 'voted').length, color: 'var(--success)' },
          { label: 'Pending Votes', value: registrations.filter(r => r.status === 'finalized').length, color: 'var(--accent2)' },
          { label: 'Waitlisted', value: registrations.filter(r => r.status === 'waitlisted').length, color: 'var(--warning)' },
      ...
      {/* Pending votes alert */}
      {registrations.filter(r => r.status === 'finalized' && r.elections?.status === 'active').length > 0 && (
        <div className="alert alert-success" style={{ marginBottom: '24px' }}>
          🎉 You have <strong>{registrations.filter(r => r.status === 'finalized' && r.elections?.status === 'active').length}</strong> election(s) waiting for your vote!
        </div>
      )}

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <h2 style={{ fontSize: '18px', fontWeight: 600 }}>My Elections</h2>
      ...
      {loading ? <LoadingSpinner /> : registrations.length === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: '64px' }}>
          ...
          <p style={{ color: 'var(--text2)', margin: '8px 0 20px' }}>Browse elections and click "I Want to Participate"</p>
          ...
          {registrations.map(r => (
            <div key={r.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: '13px', color: 'var(--text2)' }}>{r.elections?.category} • {new Date(r.elections?.start_time).toLocaleDateString()}</p>
                {r.status === 'finalized' && r.secret_code && (
                  <p style={{ fontSize: '12px', color: 'var(--accent2)', marginTop: '4px', fontFamily: 'monospace' }}>
                    🔑 Secret code ends in: ****{r.secret_code.slice(-4)}
                  </p>
                )}
              </div>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {r.status === 'finalized' && r.elections?.status === 'active' && (
                  <button onClick={() => navigate(`/vote/${r.elections?.id}`)} className="btn btn-purple">
                    🗳️ Vote Now
                  </button>
                )}
...
export function VotingPage() {
  ...
  function verifyCode() {
    ...
    if (secretInput.trim().toUpperCase() !== registration.secret_code.toUpperCase()) {
      toast.error('❌ Invalid secret code. Please check your email/notifications.');
      return;
    }
    setStep('vote');
  ...
  async function castVote() {
    ...
    try {
      ...
      // Mark voter as voted (no voter ID in votes table = anonymous)
      await supabase.from('voter_registrations').update({ status: 'voted', voted_at: new Date().toISOString() }).eq('id', registration.id);
      // Audit log (action only, no candidate info for anonymity)
      await logAudit(user.id, profile?.full_name, 'vote_cast', 'election', id, { anonymous: true });
  ...
  if (election.status !== 'active') return (
    <div style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div className="card" style={{ maxWidth: '400px', width: '100%', textAlign: 'center', padding: '48px' }}>
        <div style={{ fontSize: '48px', marginBottom: '12px' }}>⏸️</div>
        <h2>Election Not Active</h2>
        <p style={{ color: 'var(--text2)', margin: '8px 0 20px' }}>This election is currently <strong>{election.status}</strong>.</p>
  ...
  if (!registration || registration.status === 'registered' || registration.status === 'waitlisted') return (
    <div style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div className="card" style={{ maxWidth: '400px', width: '100%', textAlign: 'center', padding: '48px' }}>
        ...
        <p style={{ color: 'var(--text2)', margin: '8px 0 20px' }}>You are not a finalized voter for this election.</p>
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', background: 'radial-gradient(ellipse at top, rgba(124,58,237,0.06) 0%, transparent 60%)' }}>
      <div style={{ width: '100%', maxWidth: '580px' }}>
        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          <h2 style={{ fontSize: '20px', fontWeight: 700 }}>{election.title}</h2>
          {timeLeft && <p style={{ color: step === 'vote' ? 'var(--danger)' : 'var(--text2)', fontSize: '14px', marginTop: '4px', fontFamily: 'monospace', fontWeight: 600 }}>⏰ Time remaining: {timeLeft}</p>}
        </div>
          ...
          <div className="card fade-in">
            ...
            <div className="form-group">
              ...
              <input className="form-control" placeholder="POLL-XXXX-0001" value={secretInput}
                ...
                style={{ textAlign: 'center', fontFamily: 'monospace', fontSize: '18px', letterSpacing: '0.12em' }} />
              <p className="form-hint" style={{ textAlign: 'center' }}>Check your VoteSecure notifications or email inbox</p>
            </div>
            {/* Show masked code hint */}
          ...
          <div className="card fade-in">
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              ...
              <div className="alert alert-info" style={{ marginTop: '12px', fontSize: '13px' }}>
                🔒 Your vote is completely anonymous. Select one candidate below.
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px' }}>
              {candidates.map((c, i) => (
                <div key={c.id} className={`candidate-vote-card ${selectedCandidate === c.id ? 'selected' : ''}`}
                  ...
                  <div style={{ width: '52px', height: '52px', borderRadius: '50%', background: 'linear-gradient(135deg,var(--accent),var(--accent2))', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', fontSize: '22px', border: `2px solid ${selectedCandidate === c.id ? 'var(--accent2)' : 'var(--border)'}` }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {c.manifesto && <div style={{ fontSize: '12px', color: 'var(--text2)', marginTop: '4px' }}>{c.manifesto.substring(0, 80)}{c.manifesto.length > 80 ? '...' : ''}</div>}
                  </div>
                  <div style={{ width: '24px', height: '24px', borderRadius: '50%', border: `2px solid ${selectedCandidate === c.id ? 'var(--accent2)' : 'var(--border2)'}`, background: selectedCandidate === c.id ? 'var(--accent2)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {selectedCandidate === c.id && <span style={{ color: 'white', fontSize: '13px', fontWeight: 700 }}>✓</span>}
                  </div>
                </div>
              ))}
            </div>
            ...
            <p style={{ fontSize: '12px', color: 'var(--text3)', textAlign: 'center', marginTop: '8px' }}>
              This action is irreversible. Your vote is anonymous and encrypted.
            </p>
          </div>
        )}
        ...
        {step === 'confirmed' && (
          <div className="card fade-in" style={{ textAlign: 'center', padding: '56px 32px' }}>
            ...
            <h1 style={{ fontSize: '26px', marginBottom: '8px' }}>Vote Cast Successfully!</h1>
            <p style={{ color: 'var(--text2)', marginBottom: '32px', lineHeight: 1.8 }}>
              Your anonymous vote has been securely recorded.<br />Thank you for participating in democracy!
            </p>
